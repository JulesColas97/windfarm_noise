...
subroutine read_param(case_name,var0,simu,src,imp,output,pml,flow,tinput)
    ...
    namelist/input/nb_freq, nb_theta, nb_receiver ,theta, frequencies, Lx1,Lx2,Ly,Lz,dx,cfl,external_flow,interpolation,&
                    uniform, logarithmic,u0, arbitrary,arbitrary_new,continuation,dout, nbout, nb_receiver, heights, side, top, &
...
subroutine write_solution(case_name,theta,freq,deltaL_vect)
    use hdf5
    use utils
    implicit none
    ...
    real, dimension(:,:),intent(in)                         :: deltaL_vect
    ...
    nx = size(deltaL_vect,1)
    ny = size(deltaL_vect,2)
  ...
  !  call write_2Darray(aimag(p_vect), ceiling((1.*nx)/nbout), ceiling((1.*ny)/nbout),'p_imag',group_id)
    call write_2Darray(deltaL_vect, nx, ny,'deltaL',group_id)
    ! close group
...
subroutine write_2Dflow(u,v, epsilon, x, z)
    use hdf5
    use utils
    use types
    implicit none
    ...
    ! input variables
    real,dimension(:,:), intent(in)                           :: u, v, epsilon
...
subroutine read_h5flow3D(dirname,fname,flow3D, tinput) !,ratio,Lx, Ly, Lz,z_i, delta, flow3D)
  use hdf5
  use utils
  use types
  implicit none
