...
if (simu%arbitrary .or. simu%arbitrary_new) then
  ...
  call interp_meanflow_arbitrary(kk,simu,mesh,flow3D,tinput,flow2D,dom%nx_coarse,dom%nz_coarse,var0%c)
else
...
call  write_2Dflow(flow2D%u,flow2D%v, flow2D%c, flow2D%x, flow2D%z)
...
if (simu%arbitrary .or. simu%arbitrary_new) then
  ! interpolation and calculation of value for arbitrary mach number
  call init_meanflow_arbitrary2(kk,simu,mesh,flow2D,output%nbout,dom%nx,dom%nz,var0%c)
