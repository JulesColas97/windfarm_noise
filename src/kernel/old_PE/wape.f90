          ...
          elseif (kk<dom%nx)  then
            ...
            do ii=1,size(output%xplane)
              if (output%xplane(ii)>=0) then
                if ((mesh%x(kk)*cos(theta_rad)<=output%xplane(ii)) .and. (mesh%x(kk+1)*cos(theta_rad)>=output%xplane(ii))) then 
            ...
            do ii=1,size(output%yplane)
              if (output%yplane(ii)>0) then 
                if ((mesh%x(kk)*sin(theta_rad)<=output%yplane(ii)) .and. (mesh%x(kk+1)*sin(theta_rad)>=output%yplane(ii))) then 
   ...
   !m%h22 = 0.5
    ...
    do kk = 2,dom%nx
        ...
        if(mod(kk-1,nbout) == 0) then
            ...
            m%h22 = 0.5/(1+flow2D%epsilon(index,:))**0.5 - 0.25*flow2D%tau(index,:)/((flow2D%gamma(index,:)**2)*(1+flow2D%epsilon(index,:)))
