...
if (simu%external_flow) then
  if (simu%interpolation) then
      call read_h5flow3D(flow3D%fdir,flow3D%fname,flow3D, tinput,var0%c)
  endif
endif


! call init_output_plane(simu,dom,output,sol)
...
call init_meanflow_arbitrary(kk,simu,mesh,flow3D,tinput,flow2D,dom%nx_coarse,dom%nz_coarse,var0%c)
! else
...
call  write_2Dflow(flow2D%u, flow2D%c, flow2D%x, flow2D%z)
...
! interpolation of the meanflow over the coarse grid
!-------------------------------------------------------------------------
print*,  'interp meanflow...'
call interp_meanflow_arbitrary(kk,simu,mesh,flow2D,output%nbout,dom%nx,dom%nz,var0%c)

! Init vector for solution
!-----------------------------------------------------------------------
! to save the full plane solution 
! check if plane coresponds to propagation angle
do jj=1,size(output%xplane)
  if ((output%xplane(jj)==0) .and. ((simu%theta(kk)==90) .or. (simu%theta(kk)==-90) .or. (simu%theta(kk)==270))) then 
    output%side = .true. .or. output%side
    print*, 'output%side set to true'
  endif 
enddo

do jj=1,size(output%yplane)
  if ((output%yplane(jj)==0) .and. ((simu%theta(kk)==0) .or. (simu%theta(kk)==180) .or. (simu%theta(kk)==-180))) then 
    output%side = .true. .or. output%side
...
! to save solution at a given position to reconstruct planes
if (size(output%xplane)>0) then 
  allocate(sol%xplane(size(output%xplane),dom%nz_coarse))
  allocate(sol%xcount(size(output%xplane)))
  allocate(sol%ycoord(size(output%xplane)))
  sol%xcount = 0
endif

if (size(output%yplane)>0) then 
  allocate(sol%yplane(size(output%yplane),dom%nz_coarse))
...
! to save solution on a line of receiver a z=cst 
...
if (simu%arbitrary) then
  ...
  call  propa_1angle_1freq_arbitrary(var0,simu%frequencies(ii),simu%theta(kk),src,imp,dom,pml,flow2D, mesh, sol, output)
...
if (output%side) then
  call write_solution(case_name,simu%theta(kk),simu%frequencies(ii),sol)
endif 

call write_receiver(case_name,simu%theta(kk),simu%frequencies(ii),dom%nx_coarse,&
              & output%heights, sol%receiver_int)

call  write_planes(case_name,simu%theta(kk),simu%frequencies(ii),sol,output)
...
if (size(output%xplane)>0) then 
  deallocate(sol%xplane)
  deallocate(sol%xcount)
  deallocate(sol%ycoord)
endif

if (size(output%yplane)>0) then 
  deallocate(sol%yplane)
  deallocate(sol%ycount)
  deallocate(sol%xcoord)
endif 
...
if (simu%external_flow) then 
deallocate(flow2D%uKnot)
deallocate(flow2D%utx)
deallocate(flow2D%uty)
deallocate(flow2D%cKnot)
deallocate(flow2D%ctx)
deallocate(flow2D%cty)
endif 
