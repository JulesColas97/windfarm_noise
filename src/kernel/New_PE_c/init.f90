...
public ::  interp_meanflow_arbitrary
contains
...
!! @param[inout]    dom: parameters of the 2D mesh coresponding to the kk angles
!! @return
subroutine init_meanflow_arbitrary(kk, simu,mesh,flow3D,tinput, flow2D,  nx, nz, c0)
    use types
    use interpo
    use read_write
    implicit none
    ...
    print*, '** init 2D coarse mean flow '
    allocate(flow2D%u(nx,nz))
    allocate(flow2D%c(nx,nz))
    ...
    if (simu%external_flow) then
        ...
        allocate(flow2D%c(nx,nz))
        do ii = 1,nx
        flow2D%u(ii,:) = (simu%u0/0.4)*log((mesh%z_coarse+0.1)/0.1)
        enddo
        flow2D%c = c0
    else
        print*,'No flow'
...
subroutine interp_meanflow_arbitrary(kk, simu,mesh,flow2D, nbout, nx, nz, c0)
    use types
    use interpo
    use read_write
    implicit none
    ...
    allocate(flow2D%c(ceiling((1.*nx)/nbout),nz))
    if (simu%external_flow) then
