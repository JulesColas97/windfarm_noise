...
!
! Copied by A. Emmanuelli on 30/03/2020 from : https://github.com/certik/fortran-utils/blob/master/src/splines.f90

use types_splines, only: dp
...
!   if (info /= 0) call stop_error("spline3pars error: dgesv error.")   ! MODIF 10/04/2020
   ce=bemat(:,1)
   d2p1=2*ce(3)
end if
   ! right end
...
!if (info /= 0) call stop_error("spline3pars error: dgbsv error.")   ! MODIF 10/04/2020
cs=bmat(:,1)
!write(*,*) cs(1:6)
