...
subroutine read_param(case_name,var0,simu,src,imp,output,pml,flow,tinput)
    ...
    namelist/input/nb_freq, nb_theta, nb_receiver ,theta, frequencies, &
                    Lx1,Lx2,Ly1,Ly2,Lz,dx,cfl,external_flow,interpolation, &
                    uniform, logarithmic,u0, arbitrary,arbitrary_new, &
                    continuation,dout, nbout, nb_receiver, heights,xplane,yplane, &
                    nb_xplane,nb_yplane, side, top, &
...
subroutine write_2Dflow(u, c, x, z)
    use hdf5
    use utils
    use types
    implicit none
...
subroutine read_h5flow3D(dirname,fname,flow3D, tinput, c0) !,ratio,Lx, Ly, Lz,z_i, delta, flow3D)
  ...
  if (file_exists .eqv. .true.) then
  if (.not. allocated(p)) allocate(p(nz))
  do ii = 1,(nx+1)
    do jj = 1,(ny+1)
        ! conversion from temperature to sound speed 
        p = 1013.5*exp(-flow3D%z * 29e-3*9.81/8.314/293)
        flow3D%c(ii,jj,:) = flow3D%c(ii,jj,:) * (p/1000)**(0.286) * tinput%T_scale_K
    enddo
  enddo
        ! ---> old version
        ! flow3D%c(ii,jj,:) = flow3D%c * (1013.25/1000)**(0.286) * tinput%T_scale_K
